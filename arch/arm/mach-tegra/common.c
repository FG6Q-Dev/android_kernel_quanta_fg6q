...
void tegra_assert_system_reset(char mode, const char *cmd)
{
	...
	if (cmd) {
		...
		else if(!strcmp(cmd, "charging"))
			reg |= CHARGING_MODE;
		else
			reg &= ~(BOOTLOADER_MODE | RECOVERY_MODE | FORCED_RECOVERY_MODE | CHARGING_MODE);
	}
	else {
...
static void __init tegra_ramrepair_init(void)
{
	if (tegra_spare_fuse(10)  | tegra_spare_fuse(11)) {
		...
		printk(KERN_ERR "RYINFO: RAM_REPAIR\n");
	}
}
#endif
