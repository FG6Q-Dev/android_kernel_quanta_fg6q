...
#define GEN3_CHARGE_LED_G		TEGRA_GPIO_PK6
...
int gen3_touch_power(int on)
{
	static struct regulator *vdd_3v3_lcd_ts;
	int ret;

	if(!vdd_3v3_lcd_ts)
	{
		vdd_3v3_lcd_ts = regulator_get(NULL, "vdd_3v3_lcd_ts");
		if (IS_ERR_OR_NULL(vdd_3v3_lcd_ts)) {
			pr_err("vdd_3v3_lcd_ts regulator get failed\n");
			vdd_3v3_lcd_ts = NULL;
			return -EINVAL;
		}
	}
	...
	if(on)
	{
		ret = regulator_enable(vdd_3v3_lcd_ts);
		if(ret < 0)
		{
			pr_err("vdd_3v3_lcd_ts regulator enable failed\n");
			return ret;
		}
	...
	{
		ret = regulator_disable(vdd_3v3_lcd_ts);
		if(ret < 0)
		{
			pr_err("vdd_3v3_lcd_ts regulator disable failed\n");
			return ret;
		}
...
int __init gen3_touch_init(void)
{
...
static void gen3_hub_init()
{
	...
	gpio_request(TEGRA_GPIO_PP1, "hub_reset");
	gpio_direction_output(TEGRA_GPIO_PP1, 0);
	msleep(100);
	gpio_direction_output(TEGRA_GPIO_PP1, 1);
}

void set_charge_led(int r_on, int g_on)
{
	static int init;
...
#define GEN3_CHG_EN	TEGRA_GPIO_PI4
#define GEN3_CHG_I0		TEGRA_GPIO_PI5
#define GEN3_CHG_I1		TEGRA_GPIO_PW5
#define GEN3_CHG_VOLT	TEGRA_GPIO_PQ6

void enable_usb_charge(int enable, int current_ma, int voltage_mv)
{
	static int init;

	if(!init)
	{
		gpio_request(GEN3_CHG_EN, "charge enable");
		gpio_request(GEN3_CHG_I0, "charge I0");
		gpio_request(GEN3_CHG_I1, "charge I1");
		gpio_request(GEN3_CHG_VOLT, "charge voltage");
		init = 1;
	}
	...
	{
	case 100:
		gpio_direction_output(GEN3_CHG_I0, 0);
		gpio_direction_output(GEN3_CHG_I1, 0);
		break;
	case 500:
		gpio_direction_output(GEN3_CHG_I0, 1);
		gpio_direction_output(GEN3_CHG_I1, 0);
		break;
	case 1100:
		gpio_direction_output(GEN3_CHG_I0, 0);
		gpio_direction_output(GEN3_CHG_I1, 1);
		break;
	case 1500:
		gpio_direction_output(GEN3_CHG_I0, 1);
		gpio_direction_output(GEN3_CHG_I1, 1);
	...
	{
		case 4000:
		gpio_direction_output(GEN3_CHG_VOLT, 0);
		break;
		case 4200:
		gpio_direction_output(GEN3_CHG_VOLT, 1);
		break;
	}

	gpio_direction_output(GEN3_CHG_EN, !enable);
}
