...
//add to aviod pop noise when system power off,jimmy+++
...
int rt5640_headset_detect(struct snd_soc_codec *codec, int jack_insert)
{
	...
	if (jack_insert) {
		if (SND_SOC_BIAS_OFF == codec->dapm.bias_level) {
			snd_soc_write(codec, RT5640_PWR_ANLG1, RT5640_PWR_LDO2|RT5640_PWR_MB);
			//snd_soc_write(codec, RT5640_MICBIAS, RT5640_MIC1_CLK_EN|RT5640_MIC2_CLK_EN|RT5640_MIC1_OVCD_EN|
			//                     RT5640_PWR_MB_PU|RT5640_PWR_CLK25M_PU);
			snd_soc_write(codec, RT5640_GEN_CTRL1 , 0x3701);
		}

		//fix bug, audio will have noise when plug in HP/HS. Don't change clk,jimmy++
		//sclk_src = snd_soc_read(codec, RT5640_GLB_CLK) &
		//	RT5640_SCLK_SRC_MASK;
		//snd_soc_update_bits(codec, RT5640_GLB_CLK,
		...
		snd_soc_update_bits(codec, RT5640_PWR_ANLG2,
			RT5640_PWR_BST4|RT5640_PWR_MB1, RT5640_PWR_BST4|RT5640_PWR_MB1);
		snd_soc_update_bits(codec, RT5640_MICBIAS,
			RT5640_MIC1_OVCD_MASK ,RT5640_MIC1_OVCD_EN );
		snd_soc_update_bits(codec, RT5640_DEPOP_M1,RT5640_HP_CO_MASK|RT5640_HP_CP_MASK|RT5640_HP_CB_MASK,RT5640_HP_CO_EN|RT5640_HP_CP_PU|RT5640_HP_CB_PU);		
		snd_soc_update_bits(codec, RT5640_DEPOP_M2,RT5640_DEPOP_MASK,RT5640_DEPOP_MAN);

		snd_soc_update_bits(codec, RT5640_GEN_CTRL1,0x1, 0x1);
		msleep(200);
		if (snd_soc_read(codec, RT5640_IRQ_CTRL2) & 0x8){
#ifdef READ_ADC
			palmas_gpadc_conversion2(2, &result);
			if(result.result<=64){
				printk("============result = %d\n", result.result);
				jack_type = RT5640_MICONLY_DET;
				dev_info(codec->dev, "jack_type(mic only)\n");
			}else
#endif
				jack_type = RT5640_HEADPHO_DET;
		}else
			jack_type = RT5640_HEADSET_DET;
		snd_soc_update_bits(codec, RT5640_IRQ_CTRL2,RT5640_MB1_OC_CLR, 0);
		//fix bug, audio will have noise when plug in HP/HS. Don't change clk,jimmy++
		//snd_soc_update_bits(codec, RT5640_GLB_CLK,RT5640_SCLK_SRC_MASK, sclk_src);
		//jimmy---
	} else {
		snd_soc_update_bits(codec, RT5640_MICBIAS,
	...
	dev_info(codec->dev, "jack_type(%d)\n",jack_type);
...
static const char *micbias_ctl_text[] = {
	"600uA", "1500uA","2000uA" };

static const SOC_ENUM_SINGLE_DECL(micbias_ctl_enum, RT5640_MICBIAS, RT5640_MIC1_OVTH_SFT, micbias_ctl_text);


static const char *micbias_level_text[] = {
	"0.9", "0.75" };

static const SOC_ENUM_SINGLE_DECL(micbias_level_enum, RT5640_MICBIAS, RT5640_MIC1_BS_SFT, micbias_level_text);

static const char *spoclsd_ratio_text[]={
	"0.5x",	"0.6x", "0.71x","0.84x","1.0x","1.1x","1.2x","1.3x"};

static const SOC_ENUM_SINGLE_DECL(spkomix_volume_enum, RT5640_SPO_CLSD_RATIO, RT5640_SPO_CLSD_RATIO_SFT, spoclsd_ratio_text);
...
static const struct snd_kcontrol_new rt5640_snd_controls[] = {
	...
	SOC_DOUBLE_EXT_TLV("Speaker Playback Volume", RT5640_SPK_VOL,
		RT5640_L_VOL_SFT, RT5640_R_VOL_SFT, VOL_RESCALE_MAX_VOL, 0,
	...
	SOC_DOUBLE_TLV("IN Capture Volume", RT5640_INL_INR_VOL,
			...
			31, 1, in_vol_tlv),
	/* Speaker detection */
	SOC_SINGLE("Speaker detection Switch", RT5640_PV_DET_SPK_G,
		RT5640_PVDD_DET_SFT, 1, 0),

	/* Speaker auto gain */
	SOC_SINGLE("Speaker auto gain", RT5640_PV_DET_SPK_G,
		RT5640_SPK_AG_SFT, 1, 0),
	...
	SOC_DOUBLE_TLV("ADC Boost Gain", RT5640_ADC_BST_VOL,
			...
			3, 0, adc_bst_tlv),
	SOC_ENUM("MIC BIAS delay control", micbias_ctl_enum),
	SOC_ENUM("MIC BIAS level control", micbias_level_enum),
	SOC_ENUM("SPKOMIX CLSD Ratio Control", spkomix_volume_enum),
	/* Class D speaker gain ratio */
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		rt5640_index_update_bits(codec, RT5640_CLSD_INT_REG1, 0xf000, 0xf000);
	...
	case SND_SOC_DAPM_PRE_PMD:
		...
		rt5640_index_update_bits(codec, RT5640_CLSD_INT_REG1, 0xf000, 0x0000);
...
static int rt5640_probe(struct snd_soc_codec *codec)
{
	...
	if ((val != rt5640_reg[RT5640_RESET]) && (val != RT5639_RESET_ID)) {
		dev_err(codec->dev,
			"Device with ID register %x is not rt5640/39(1st)\n", val);
		
		//workaround for codec probe fail
		msleep(50);
		val = snd_soc_read(codec, RT5640_RESET);
	        if ((val != rt5640_reg[RT5640_RESET]) && (val != RT5639_RESET_ID)) {
        	        dev_err(codec->dev,
                	        "Device with ID register %x is not rt5640/39(2nd)\n", val);
					mutex_unlock(&rt5640->lock);
					return -ENODEV;
		}
		dev_err(codec->dev,"Get Device ID again success\n");
	}
...
static void rt5640_i2c_shutdown(struct i2c_client *i2c)
{
	...
	if( gCodec != NULL ) {
			if( &gCodec->dapm != NULL ) {
				snd_soc_dapm_disable_pin(&gCodec->dapm, "Int Spk");
				snd_soc_dapm_disable_pin(&gCodec->dapm, "Headphone Jack");
				snd_soc_dapm_disable_pin(&gCodec->dapm, "Mic Jack");
				snd_soc_dapm_disable_pin(&gCodec->dapm, "Int Mic");
				snd_soc_dapm_sync(&gCodec->dapm);
				msleep(200);
				rt5640_set_bias_level(gCodec, SND_SOC_BIAS_OFF);
			}
	}
	//jimmy---
