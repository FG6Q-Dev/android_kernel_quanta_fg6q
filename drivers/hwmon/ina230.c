...
static struct regulator *vdd_lcd_hv_r_edp;
...
static s32 power_down_ina230(struct i2c_client *client)
{
	...
	if(vdd_lcd_hv_r_edp)
		regulator_disable(vdd_lcd_hv_r_edp);
...
s32 ina230_get_shunt_voltage(s32 *voltage_mV)
{
...
s32 ina230_get_shunt_voltage_uv(s32 *voltage)
{
	struct i2c_client *client = g_client;
	struct ina230_data *data = i2c_get_clientdata(client);
	s32 voltage_uV;
...
//request SMPS6 power or the I2C Gen2 3V pull high will be removed in early suspend
	...
	{
		dev_err(&client->dev, "request regulator vdd_lcd_hv_r_edp fail\n");
		vdd_lcd_hv_r_edp = NULL;
		goto exit;
	}
	if(vdd_lcd_hv_r_edp)
		regulator_enable(vdd_lcd_hv_r_edp);
#endif
