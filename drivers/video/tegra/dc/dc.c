...
int _tegra_dc_update_cmu(struct tegra_dc *dc, struct tegra_dc_cmu *cmu)
{
	...
	if(cmu != &dc->cmu){	
		...
		dev_info(&dc->ndev->dev,"cmu matrix not changed\n");
	}
...
void tegra_dc_load_lut2(struct tegra_dc *dc)
{
	...
	snprintf(fw_name,sizeof(fw_name),"cmu_lut2_%d",dc->ndev->id);
	if(request_firmware(&fw_entry,fw_name,&dc->ndev->dev)){
		dev_err(&dc->ndev->dev, "request_firmware %s fail\n",fw_name);
		return;
	}else{
		dev_info(&dc->ndev->dev, "%s data size: %d\n",fw_name,fw_entry->size);
	}

	data_str = kzalloc((fw_entry->size+1),GFP_KERNEL);
	memcpy(data_str,fw_entry->data,fw_entry->size);
	data_str[fw_entry->size]='\0';

	data_sep = argv_split(GFP_KERNEL,data_str,&cnt);
	for( i = 0; i < cnt; i++ ){
		if( data_sep[i] ){
			ret = kstrtou8( data_sep[i],10,&data_nm[i]);
			//printk(KERN_INFO "firmware data [%d] in number: %#x vs in str: %s\n",i,data_nm[i],data_sep[i]);
		}
	}

	if(!dc->pdata->cmu_cali){
		dc->pdata->cmu_cali = kzalloc(sizeof(struct tegra_dc_cmu),GFP_KERNEL);
	}
	memcpy(dc->pdata->cmu_cali,&dc->cmu,sizeof(struct tegra_dc_cmu));



	for( i = 0; i < 960; i++ ){
		dc->pdata->cmu_cali->lut2[i] = data_nm[i];
	}
	//tegra_dc_update_cmu(dc,dc->pdata->cmu_cali);
...
EXPORT_SYMBOL(tegra_dc_load_lut2);


void tegra_dc_load_csc(struct tegra_dc *dc)
{
	const struct firmware *fw_entry;
	char fw_name[16];
	char *data_str;
	...
	snprintf(fw_name,sizeof(fw_name),"cmu_csc_%d",dc->ndev->id);
	if(request_firmware(&fw_entry,fw_name,&dc->ndev->dev)){
		dev_err(&dc->ndev->dev, "request_firmware %s fail\n",fw_name);
		return;
	}else{
	...
	memcpy(data_str,fw_entry->data,fw_entry->size);
	data_str[fw_entry->size]='\0';
	//printk(KERN_INFO "firmware cmu_csc data: str: %s\n",data_str);

	data_sep = argv_split(GFP_KERNEL,data_str,&cnt);
	for( i = 0; i < cnt; i++ ){
		if( data_sep[i] ){
			ret = kstrtou16( data_sep[i],16,&data_nm[i]);
			dev_info(&dc->ndev->dev, "%s [%d] in number: %#x vs in str: %s\n",
					fw_name,i,data_nm[i],data_sep[i]);
		}
	}
	...
	memcpy(dc->pdata->cmu_cali,&dc->cmu,sizeof(struct tegra_dc_cmu));
	dc->pdata->cmu_cali->csc.krr = data_nm[0];
	dc->pdata->cmu_cali->csc.kgr = data_nm[1];
	dc->pdata->cmu_cali->csc.kbr = data_nm[2];
	dc->pdata->cmu_cali->csc.krg = data_nm[3];
	dc->pdata->cmu_cali->csc.kgg = data_nm[4];
	dc->pdata->cmu_cali->csc.kbg = data_nm[5];
	dc->pdata->cmu_cali->csc.krb = data_nm[6];
	dc->pdata->cmu_cali->csc.kgb = data_nm[7];
	dc->pdata->cmu_cali->csc.kbb = data_nm[8];
...
EXPORT_SYMBOL(tegra_dc_load_csc);

void tegra_dc_cmu_enable(struct tegra_dc *dc, bool cmu_enable)
{
	if(cmu_enable){
		tegra_dc_load_lut2(dc);
		tegra_dc_load_csc(dc);
	}
	
	
	dc->pdata->cmu_enable = cmu_enable;
	if( dc->pdata->cmu_cali){	
		dev_info(&dc->ndev->dev, "%s: use calibrated cmu data\n",__func__);
		tegra_dc_update_cmu(dc, dc->pdata->cmu_cali);
	}else if (dc->pdata->cmu){
		dev_info(&dc->ndev->dev, "%s use pdata defined cmu data\n",__func__);
		tegra_dc_update_cmu(dc, dc->pdata->cmu);
	}else{
		dev_info(&dc->ndev->dev, "%s use default cmu data\n",__func__);
...
#ifdef CONFIG_TEGRA_DC_CMU
	if(dc->pdata->cmu_cali){
		dev_info(&dc->ndev->dev, "use calibrated cmu data\n");
		_tegra_dc_update_cmu(dc, dc->pdata->cmu_cali);	
	}else if (dc->pdata->cmu){
		dev_info(&dc->ndev->dev, "use pdata defined cmu data\n");
		_tegra_dc_update_cmu(dc, dc->pdata->cmu);
	}else{
		dev_info(&dc->ndev->dev, "use default cmu data\n");
