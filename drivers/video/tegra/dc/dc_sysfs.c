...
static ssize_t cmu_csc_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct platform_device *ndev = to_platform_device(dev);
	struct tegra_dc *dc = platform_get_drvdata(ndev);
...
static ssize_t cmu_csc_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct platform_device *ndev = to_platform_device(dev);
	struct tegra_dc *dc = platform_get_drvdata(ndev);
	...
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.krr);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kgr);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kbr);
	res += snprintf(buf+res,PAGE_SIZE-res,"\n");

	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.krg);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kgg);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kbg);
	res += snprintf(buf+res,PAGE_SIZE-res,"\n");

	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.krb);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kgb);
	res += snprintf(buf+res, PAGE_SIZE-res, "%#4X ", (u16)dc->cmu.csc.kbb);
	res += snprintf(buf+res,PAGE_SIZE-res,"\n");

	return res;
}

static DEVICE_ATTR(cmu_csc,
		S_IRUGO|S_IWUSR, cmu_csc_show, cmu_csc_store);



static ssize_t cmu_lut2_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct platform_device *ndev = to_platform_device(dev);
	struct tegra_dc *dc = platform_get_drvdata(ndev);
...
static ssize_t cmu_lut2_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct platform_device *ndev = to_platform_device(dev);
	struct tegra_dc *dc = platform_get_drvdata(ndev);
	...
	for( j = 0 ; j < matrix_h ; j++ ){
		for( i = 0 ; i < matrix_w ; i++ ){
			res += snprintf(buf+res, PAGE_SIZE-res, "%u ", dc->cmu.lut2[i+matrix_w*j]);
		}
...
static DEVICE_ATTR(cmu_lut2,
		S_IRUGO|S_IWUSR, cmu_lut2_show, cmu_lut2_store);
...
void tegra_dc_create_sysfs(struct device *dev)
{
	...
	error |= device_create_file(dev, &dev_attr_cmu_csc);
	error |= device_create_file(dev, &dev_attr_cmu_lut2);
#endif
