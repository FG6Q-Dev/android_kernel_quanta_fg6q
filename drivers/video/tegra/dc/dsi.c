...
static u8 sharp_ntsc[] = {0x32, 0x00, 0x02};
static struct tegra_dsi_cmd dcs_cmd_ntsc = DSI_CMD_LONG(DSI_GENERIC_LONG_WRITE, sharp_ntsc);
#endif
...
{
	u32 i;
	struct tegra_dc_dsi_data *dsib = tegra_dsi_instance[1];

	/* Skip LINKB regulator enable since we did it in
	 * LINKA already.
	 * */
	if(dsi != dsib) {
					...
					u32 n_cmd)
{
	...
	for (i = 0; i < n_cmd; i++) {
		...
		cur_cmd = &cmd[i];
dev_info(&dc->ndev->dev,"===Send DCS command: data_id, data_len: %x: %x\n",
					cur_cmd->data_id, cur_cmd->sp_len_dly.data_len);
...
//			tegra_dsi_send_dc_frames(dc, dsi, 2);
		...
		err = tegra_dsi_write_data(dc, dsi,
							dcs_cmd_ntsc.pdata,
							dcs_cmd_ntsc.data_id,
							dcs_cmd_ntsc.sp_len_dly.data_len);
		if (err < 0) {
			dev_err(&dc->ndev->dev, "DSI ntsc write failed\n");
		}
