...
#define IMX091_LENS_VIEW_ANGLE_H	61000 /* / _INT2FLOAT_DIVISOR */
#define IMX091_LENS_VIEW_ANGLE_V	47000 /* / _INT2FLOAT_DIVISOR */
...
static struct nvc_gpio_init imx091_gpios[] = {
	...
	{IMX091_GPIO_PWDN, GPIOF_OUT_INIT_LOW, "pri_cam_pwdn", false, true},
};
...
static int imx091_i2c_rd_eeprom_afc(struct imx091_info *info, u16 reg, u16 *val)
{
	struct i2c_msg msg[2];
	u8 buf[4];
...
static int imx091_dev_id(struct imx091_info *info)
{
	...
	err = imx091_i2c_rd_eeprom_afc(info, 0x3ffa, &val);
	if (!err) {
		pr_info("%s imx091_i2c_rd_eeprom_afc, 0x3ffa ==> 10cm: %x\n", __func__, val);
	}

	err = imx091_i2c_rd_eeprom_afc(info, 0x3ffc, &val);
	if (!err) {
		pr_info("%s imx091_i2c_rd_eeprom_afc, 0x3ffc ==> 40m: %x\n", __func__, val);
	}
