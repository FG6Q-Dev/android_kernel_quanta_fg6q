...
#define AD5816_FNUMBER_FLOAT		(2.2f)
#define AD5816_FOCAL_LENGTH			(0x40923D71) /* 4.570f */
#define AD5816_FNUMBER				(0x400CCCCD) /* 2.2f */
#define AD5816_SLEW_RATE		1
#define AD5816_ACTUATOR_RANGE		1023
#define AD5816_SETTLETIME		20

#if defined(CONFIG_PROJECT_SKU_VARIANT_BBG)
#define AD5816_FOCUS_MACRO		800
#define AD5816_FOCUS_INFINITY		300
#elif defined(CONFIG_PROJECT_SKU_VARIANT_LEPAN)
#define AD5816_FOCUS_MACRO		540
#define AD5816_FOCUS_INFINITY		150
#elif defined(CONFIG_PROJECT_SKU_VARIANT_GIGASET)
#define AD5816_FOCUS_MACRO		540
#define AD5816_FOCUS_INFINITY		150
#else
...
// kokob3: read auto focus calibration data from eeprom
static int ad5816_i2c_rd_eeprom_afc(struct ad5816_info *info, u16 reg, u16 *val)
{
	struct i2c_msg msg[2];
	u8 buf[4];
	...
	buf[0] = (reg >> 8);
	buf[1] = (reg & 0x00FF);
	msg[0].addr = 0x50; // 0xA0>>1, 7 bit
	msg[0].flags = 0;
	msg[0].len = 2;
	msg[0].buf = &buf[0];
	msg[1].addr = 0x50; // 0xA0>>1, 7 bit
	msg[1].flags = I2C_M_RD;
	msg[1].len = 2;
	msg[1].buf = &buf[2];
	*val = 0;
...
void ad5816_set_arc_mode(struct ad5816_info *info)
{
	...
	/* set the ARC RES2 */
	err = ad5816_i2c_wr8(info, MODE, 0x43);
	if (err)
		dev_err(&info->i2c_client->dev,
		"%s: MODE reg write failed\n", __func__);

	/* set the VCM_FREQ : Tres = 10.86ms fres = 92Hz */
	err = ad5816_i2c_wr8(info, VCM_FREQ, 0x43);
	if (err)
		dev_err(&info->i2c_client->dev,
...
static void ad5816_get_focuser_capabilities(struct ad5816_info *info)
{
	...
	// kokob3: read AF calibration data from EEPROM
	pr_err("%s ++\n", __func__);
	err = ad5816_i2c_rd_eeprom_afc(info, 0x3ffa, &val);
	if (!err) {
		info->cap.focus_macro = val + 100;
	}

	err = ad5816_i2c_rd_eeprom_afc(info, 0x3ffc, &val);
	if (!err) {
		info->cap.focus_infinity = val - 100;
	}
	pr_info("%s update info->cap.focus_macro ==> %x\n", __func__, info->cap.focus_macro);
	pr_info("%s update info->cap.focus_infinity ==> %x\n", __func__, info->cap.focus_infinity);
...
static void ad5816_sdata_init(struct ad5816_info *info)
{
	...
	pr_info("%s sdata info->cap.focus_macro ==> %x\n", __func__, info->cap.focus_macro);
	pr_info("%s sdata info->cap.focus_infinity ==> %x\n", __func__, info->cap.focus_infinity);
